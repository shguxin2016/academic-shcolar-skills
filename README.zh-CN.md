...
Academic Research Skills 是一套面向 AI 编程代理的学术研究到论文工作流套件。本清理镜像保留上游 Claude Code skills，并增加了 **Claude Code**、**Codex**、**OpenCode** 三个平台的本地入口。

> 二次分发镜像：本仓库保留上游署名和许可证条款。继续使用前请先阅读 [FORK_NOTICE.md](FORK_NOTICE.md)。许可证为 CC BY-NC 4.0，包含非商业限制。

## 支持的运行时

| 运行时 | 支持状态 | 入口 |
|---|---|---|
| Claude Code | 原生支持 | `.claude-plugin/`、`commands/`、`agents/`、四个顶层 `SKILL.md` 目录 |
| Codex | 通过单一 router skill 支持 | `codex/academic-research-suite/` 与 `scripts/install-codex-skill.*` |
| OpenCode | 通过项目 agent 配置支持 | `opencode.json`、`.opencode/agents/academic-research-suite.md`、`AGENTS.md` |

核心工作流：深度研究、文献综述、系统综述、论文计划与写作、同行评审模拟、引用检查、AI disclosure、完整 research-to-paper pipeline。
...
也可以把四个目录复制或 symlink 到项目的 `.claude/skills/`：
...
也可直接输入 `ars-plan`、`ars-outline`、`ars-lit-review`、`ars-full` 等别名。
...
OpenCode 会读取 `opencode.json`，默认 agent 是 `academic-research-suite`。可用命令包括：
...
本镜像为了二次上传合规，移除了上游 GitHub Actions、赞助元数据、第三方 HTML 快照和展示 PDF 二进制文件。核心 skills、references、scripts、examples、tests 已保留。

## 项目结构

- `deep-research/`：深度研究与文献综述。
- `academic-paper/`：论文写作、修订、引用检查与格式转换。
- `academic-paper-reviewer/`：同行评审与编辑决策模拟。
- `academic-pipeline/`：完整 research-to-paper pipeline。
- `shared/`、`scripts/`、`docs/`、`examples/`：共享协议、验证脚本、文档和示例。
...
原上游项目包含一次完整 10 阶段 pipeline 的 PDF 展示产物。本镜像为了保持仓库轻量，
并降低第三方内容二次分发的审核风险，没有重新分发这些二进制 PDF。

保留的产物清单见 [examples/showcase/README.md](examples/showcase/README.md)；如果需要原始展示文件，
请查看上游仓库。
