...
Academic Research Skills is a research-to-paper workflow suite for AI coding
agents. This cleaned mirror keeps the upstream Claude Code skills and adds local
entry points for **Claude Code**, **Codex**, and **OpenCode**.

> Redistribution mirror: this repository preserves upstream attribution and
> license terms. See [FORK_NOTICE.md](FORK_NOTICE.md) before reuse. The license
> is CC BY-NC 4.0, including the non-commercial restriction.

## Supported Runtimes

| Runtime | Status | Entry point |
|---|---|---|
| Claude Code | Native support | `.claude-plugin/`, `commands/`, `agents/`, and the four top-level `SKILL.md` folders |
| Codex | Supported through a single router skill | `codex/academic-research-suite/` and `scripts/install-codex-skill.*` |
| OpenCode | Supported through project agent config | `opencode.json`, `.opencode/agents/academic-research-suite.md`, and `AGENTS.md` |

Core workflows:

- Deep research, literature review, systematic review, meta-analysis, and source verification.
- Academic paper planning, outlining, drafting, abstract writing, revision, citation checks, disclosure, and formatting.
- Peer-review simulation, editorial decisions, methodology review, and re-review.
- End-to-end research-to-paper pipeline with integrity gates and Material Passport handoffs.

## Quick Install

See [docs/INSTALL.md](docs/INSTALL.md) for the complete setup guide.
...
Traditional project install is also supported: copy or symlink these four
...
Clone this repository, then install the bundled Codex router skill.
...
Plain aliases such as `ars-plan`, `ars-outline`, `ars-lit-review`, and
`ars-full` are routed by the Codex skill.

### OpenCode

Clone this repository and start OpenCode from the repo root:
...
OpenCode reads `opencode.json`; the default agent is
`academic-research-suite`. Available commands include:
...
This mirror intentionally removes upstream GitHub Actions, sponsorship metadata,
third-party HTML snapshots, and showcase PDF binaries. The operational skills,
references, scripts, examples, and tests needed for local use are retained.

After install, use a simple smoke test:

- Claude Code: `/ars-plan` starts paper-planning dialogue.
- Codex: `$academic-research-suite` appears in a new conversation after restart.
- OpenCode: `/ars-plan` appears in the command list from this repository.

## Project Architecture

- `deep-research/` — 13-agent research workflow.
- `academic-paper/` — academic writing and revision workflow.
- `academic-paper-reviewer/` — multi-perspective reviewer workflow.
- `academic-pipeline/` — full research-to-paper orchestrator.
- `shared/`, `scripts/`, `docs/`, `examples/` — common schemas, validators, docs, and fixtures.
## Guides & articles
...
The original upstream project includes showcase PDFs from a complete 10-stage
pipeline run. Those binary artifacts are intentionally not redistributed in this
mirror to keep the repository lightweight and reduce third-party content review
risk.

See [examples/showcase/README.md](examples/showcase/README.md) for the retained
artifact inventory, or consult the upstream repository if you need the original
showcase files.
...
>>>>>>> ec13c41 (docs: remove README contributor sections)
