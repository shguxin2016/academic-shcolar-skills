...
**Manifest:** the original upstream snapshot manifest lived at `docs/design/snapshots/2026-05-13-ai-disclosure-discovery/manifest.yaml`. This mirror does not redistribute the third-party HTML snapshots; use the Wayback IDs and upstream repository when byte-level snapshot verification is required.
...
Verification protocol (per upstream manifest): re-compute SHA-256 of the local mirror before writing a mandate/recommend cell; mismatch → re-fetch from `wayback_url` + re-hash before continuing. In this redistribution mirror, the local snapshot files are intentionally omitted.
